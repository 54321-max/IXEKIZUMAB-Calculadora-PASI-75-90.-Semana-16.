...
for endpoint, info in models_info.items():
    ...
    # Caso 1: metadata antiguo → info es string (ruta del modelo)
    if isinstance(info, str):
        st.caption(f"Archivo del modelo: {info}")
        st.info(
            "Este modelo no incluye métricas internas (AUC, Brier). "
            "Si se desea mostrarlas, es necesario reentrenar el modelo."
        )
        continue

    # Caso 2: metadata nuevo → info es diccionario con métricas
    ...
    c4.metric("Brier", f"{brier:.3f}" if isinstance(brier, (int, float)) else "NA")
        ...
        st.warning("No se encontró models_*/metadata.json. Reentrena y vuelve a subir los modelos.")
